...
public class ApkInfoHelper {
    ...
    public ApkInfoHelper() {
        ...
        if(app == null || app.isEmpty()){ //если путь к apk файлу не указан, выкидываем ошибку
            throw new RuntimeException("No value for key 'apk' providing apk path in emulator.properties");
        }
        try {
    ...
    /**
     * Метод для получения URL APK из локального пути
     * @return URL файла APK
     */
    public String getAppUrlFromLocalPath() {
        // Здесь можно использовать логику для получения локального пути к APK
        // Например, если APK файл лежит в определенной директории проекта
        return Paths.get("path/to/your/apk/file.apk").toUri().toString();
    }
}
